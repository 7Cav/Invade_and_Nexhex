...
{	
	_noAirTransportAvailable = true;
	{
		...
		{
			if ((_currentVehData select 3) in blufor_vehicle_variants_transportRotaryWing_deploymentData) then
			{
				if (alive (driver _veh)) then
				{
					_noAirTransportAvailable = false;
				};
			};
	...
	missionNamespace setVariable ["NoAirTransportAvailable", _noAirTransportAvailable, true];
